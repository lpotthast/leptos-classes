...
fn frontend_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .canonicalize()
        .expect("test frontend directory should exist")
}
...
pub async fn start_frontend() -> Result<Frontend> {
    ...
    let fe_process = Process::new(cmd)
        .spawn_broadcast()
        .context("failed to spawn `cargo leptos serve` for leptos-classes test app")?
    ...
    tracing::info!("Waiting {timeout:?} for leptos-classes test app to start...");
        ...
        .await
    {
        ...
        WaitForLineResult::StreamClosed => {
            bail!(
                "leptos-classes test app stdout closed before startup completed. Expected stdout to contain {expected_line:?}.\n\nRecent stdout:\n{}\n\nRecent stderr:\n{}",
        ...
        WaitForLineResult::Timeout => {
            bail!(
                "leptos-classes test app did not start within {timeout:?}; expected stdout to contain {expected_line:?}.\n\nRecent stdout:\n{}\n\nRecent stderr:\n{}",
                render_log_lines(&stdout_lines),
                render_log_lines(&stderr_lines),
            );
        }
